...
const AddToCartButton = ({ product }: ProductProps) => {
  ...
  return (
    <div className="flex flex-row mt-auto justify-center w-full gap-8 h-14">
      <div className="flex justify-center gap-3 items-center  p-0 m-0 flex-1/4">
        <button
          onClick={addQuantity}
          className="text-lg font-bold bg-hover-light hover:bg-border rounded-lg p-2 w-10  cursor-pointer flex-1 flex justify-center"
        >
          <Plus className="w-5 h-auto" />
        </button>
        <p className="text-lg p-2 font-bold text-center">{quantity}</p>
        <button
